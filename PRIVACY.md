...
The app does not read Codex's local thread database, enumerate project directories, or read source files.
...
Codex Rate Widget has no direct networking client and sends no telemetry or analytics. The child Codex CLI process may communicate with Codex services for the account methods described above.
...
Do not attach App Group snapshots, `Config/Local.xcconfig`, or unredacted logs to a public issue.
