...
The weekly pace guide is calculated locally from the official weekly reset timestamp and the known 10,080-minute window. The app and large widget show the same past seven-day time domain and repeat the current reset cadence backward as separate straight segments from 100% at each inferred cycle start to 0% at reset. Keeping the segments separate avoids drawing a false dotted connection from 0% to 100% across a reset. The app and widget enter their warning presentation when the current official remaining percentage is at least 10 percentage points below the current-cycle line. During that warning, the app and large-widget charts divide the current percentage-point shortfall by the guide's constant decline rate, round up to whole minutes, and display the required pause as days, hours, and minutes. The estimate assumes all account usage remains paused until the guide catches the observed remaining percentage.

The warning annotation also fits an ordinary least-squares line to the weekly remaining-capacity observations recorded between the current official cycle start and the next official `resetsAt`. Projection starts only after at least eight samples spanning six hours and rejects stale observations. A non-negative slope is reported as no downward trend. A zero crossing beyond the official reset is labeled as after reset because that reset will invalidate the extrapolation. This is a local trend estimate, not an official Codex forecast; irregular reset inference is intentionally outside the model.

The guide, warning, and pause duration are advisory UI derived from the most recently observed official current values; they are not an account limit, forecast, or historical series returned by Codex. The pause duration assumes no additional use from any machine on the account. If the weekly window or a valid current reset cycle is unavailable, no pace assessment or warning is shown.
